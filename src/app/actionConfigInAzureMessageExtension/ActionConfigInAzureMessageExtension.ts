  ...
  public async onFetchTask(context: TurnContext, value: MessagingExtensionQuery): Promise<MessagingExtensionResult | TaskModuleContinueResponse> {
    ...
    public async onQuerySettingsUrl(context: TurnContext): Promise<{ title: string, value: string }> {
      ...
      return Promise.resolve({
          ...
          value: `https://${process.env.HOSTNAME}/actionConfigInAzureMessageExtension/config.html?name={loginHint}&tenant={tid}&group={groupId}&theme={theme}&siteID=${config.SiteID}&listID=${config.ListID}&useSearch=${config.UseSearch.toString()}&contentType=${config.ContentType}`
      });
    }
    ...
    public async onSettings(context: TurnContext): Promise<void> {
        ...
        await Utilities.saveConfig({SiteID: setting.siteID, ListID: setting.listID, UseSearch: setting.useSearch, ContentType: setting.contentType});
        return Promise.resolve();
    }
}
